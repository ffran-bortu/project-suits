...
var unit_lookup: Dictionary = {} # O(1) Cache for grid position -> unit
...
func add_unit(unit_node):
	...
	# Connect to movement signal to keep cache updated
	if not unit_node.unit_moved.is_connected(_on_unit_moved):
		unit_node.unit_moved.connect(_on_unit_moved.bind(unit_node))

## Register unit (alias for add_unit, for DuoFactory compatibility)
func register_unit(unit_node):
	add_unit(unit_node)

# Data signal handler
func _on_unit_moved(old_pos: Vector2i, new_pos: Vector2i, unit):
...
## Get unit at specific grid position (O(1) lookup).
## @param grid_pos: Grid position to check
## @return: Unit node at position, or null if no unit present
func get_unit_at_position(grid_pos: Vector2i):
	# O(1) lookup
	return unit_lookup.get(grid_pos)

## Select a unit and display its movement range.
## @param unit: Unit node to select
## @return: true if selection succeeded, false if unit invalid or already acted
...
# Display movement range on grid (internal helper)
...
## Get pathfinding from selected unit to target position.
## @param target_pos: Destination grid position
## @return: Array of Vector2i positions forming path (empty if unreachable)
...
## Move selected unit to target grid position.
## Validates target is in movement range, then animates movement.
## @param target_pos: Destination grid position
...
## @param team_id: Team number (0 = player, 1 = enemy)
## @return: Array of unit nodes on that team
