"""
FILE: map_loader.gd
PURPOSE: Static parser for .txt map files - extracts height map, enemy/player positions from structured text format.
FUNCTIONS: load_map_file (static), _parse_height_row (static), _parse_positions (static), _letter_to_column (static) - 4 total
NOTES: Format: height map rows (space-separated floats), then "enemies: 1A 2B", then "players: 3C". Position format: rowLetter (1A = row 1, column A=0). Returns Dictionary with grid_size, height_map, enemies, players arrays.
"""
...
static func _parse_height_row(line: String) -> Array[float]:
...
static func _parse_positions(positions_string: String) -> Array[Vector2i]:
